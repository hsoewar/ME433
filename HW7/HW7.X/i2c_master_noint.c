...
void i2c_master_setup(void) {
    ...
    I2C1BRG = 400; // I2CBRG = [1/(2*Fsck) - TPGD]*Pblck - 2 (TPGD is the Pulse Gobbler Delay)
...
void i2c_master_read_multiple(unsigned char address, unsigned char reg, unsigned char * data, int length) {
    ...
    i2c_master_send(address); // addresses the device to send information to //need to append write
    i2c_master_send(reg); // selects the pin to read from
    i2c_master_restart();
    unsigned char addressread = address | 0b00000001;
    i2c_master_send(addressread); // addresses the device to send information to //need to append read
    for (i = 1; i <= length; i++ ) {
        data[i] = i2c_master_recv(); // get received values of all 8 pins
        if (i < length) {
            i2c_master_ack(0); // prompts reception of more data
        }
        else {
            i2c_master_ack(1); // acknowledges all values has been received
        }
    }
